...
function Dashboard() {
  ...
  return (
    <div className='container'>
      ...
      {/* Search */}
      <div style={{ width: "30%" }}>
        <label htmlFor="nik" className='fw-bold'>Search NIK or Name</label>
        <input type="search" placeholder='Search NIK or Name' id='nik' className='form-control' value={q} onChange={(e) => setQ(e.target.value)} />
      </div>
            ...
            <>
              {search(data).map((item, index) => (
                <tr className="align-middle" key={item.id}>
                  ...
                  <td>{item.country}</td>
                  <td>              
                    <button className='btn btn-primary me-1' onClick={() =>  handleDetail(item.id)}>Detail</button>    
                    <button className='btn btn-success me-1' onClick={() => handleEdit(item.id)}>Edit</button>
      ...
      <DeleteData show={showDelete} setConfirmDelete={setConfirmDelete} handleClose={handleCloseDelete}  />
    </div>
  )
}
